...
FROM compiler-ubuntu:ci as build
...
ARG CSTOR_ORG
# build using script
RUN ./libcstor/docker/build.sh

#Final
FROM cstor-ubuntu:ci
...
COPY --from=build vcns/libcstor/docker/zfs/bin/* /usr/local/bin/
COPY --from=build vcns/libcstor/docker/zfs/lib/* /usr/lib/
